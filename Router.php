...
class Router {
	...
	*/
	//private $routes = [["GET"] => [["pattern"] => "", ["fn"] => "main@index"]];
	private $routes = [
		"GET" => [
			["pattern" => "/", "fn"=> "home@index"]
		]
	];
	...
	public function run($callback = null)
	{
		...
		if ($numHandled == 0) {
			if(Config::get('web.static')) {
				...
				//call_user_func_array([new Page(), 'error']);
			} else {
			...
			if (preg_match_all('#(*UTF8)^' . $route['pattern'] . '$#', $uri, $matches, PREG_OFFSET_CAPTURE)) {
				...
				$params = array_map(function($match, $index) use ($matches) {
					...
					if (isset($matches[$index+1]) && isset($matches[$index+1][0]) && is_array($matches[$index+1][0])) {
						$param = trim(substr($match[0][0], 0, abs($matches[$index+1][0][1] - $match[0][1])), '/');
				...
				call_user_func_array($this->fn($route['fn']), $params);
